...
func (d *Device) GetDescriptor(descType int) ([]byte, error) {
	if d.handle == nil {
		return nil, fmt.Errorf("GetDescriptor(%d) called on %s after Close", descType, d)
	}

	return d.ctx.libusb.getDescriptor(d.handle, descType)
}
